...
- [x] **M246.2 — Record the judge-council product ratings and final known gap** `DONE`
  ...
  - Evidence: `docs/JUDGE_COUNCIL.md` records two product-council rounds across the official criteria and judge-role lenses. After the M246.1 fix the council recorded no known implementation defect remaining before demo packaging. Verification after the council fixes: `npm run gate` passed formatting, lint with zero warnings, typecheck, 413 tests, production build, and authorship check; `npm run evals` passed 21/21.
...
- [x] **M247.1 — Add ChatGPT Sites as a second production surface** `DONE`
  ...
  - Evidence: on 2026-09-04, `.openai/hosting.json` was added for ChatGPT Sites project `appgprj_6a99be5d07ac8191a0e1b47ff02b5bf1`, the official Sites Vite plugin was added, the build now emits a Sites-compatible `dist/server/index.js` worker with SPA fallback, and browser-side API calls use `VITE_AETHER_API_BASE_URL` when built for Sites. Railway deployment `bbb12752-934b-4fcc-8e6c-c50b4068b5f6` is live with CORS verified for the Sites origin: `/health` returned PostgreSQL persistence and `/api/live/openai` returned OpenAI status with `Access-Control-Allow-Origin: https://aether-architecture-lab.sreenath-mm89.chatgpt.site`. ChatGPT Sites version 3 was saved from commit `cbabe589bbde6a75e8d80259f6f0c3017b5f1c70` and publicly deployed to `https://aether-architecture-lab.sreenath-mm89.chatgpt.site`. Curl verified the Sites root returns `200` HTML and the JS bundle is served. Playwright browser validation loaded the live Sites URL, confirmed the Aether UI title and visible workspace, clicked “Watch an agent work,” observed persistent sync through Railway, `read_live_source` returning OpenAI status, `measure_component_demand` returning npm demand, a highest-resilience future opening, the surface expanding to eighteen tools, and the human approval control remaining disabled because the traffic-spike evidence still blocks approval. Verification: `npm run gate` passed before Sites publishing; after the Node/Vite compatibility fix, `npm run evals` passed 26/26, and `npm run format:check`, `npm run lint`, `npm run typecheck`, and the Sites-targeted build with `VITE_AETHER_API_BASE_URL=https://webmcp-production-38e5.up.railway.app npm run build` passed.
  - Update evidence: on 2026-09-04, after owner-added commits through `c090a07414c56ddd82e6b46a93b81abbdb640063`, the Sites-targeted build passed and produced bundle `assets/index-BFclHJ_V.js`. ChatGPT Sites version 4 was saved from that exact commit and publicly deployed to `https://aether-architecture-lab.sreenath-mm89.chatgpt.site`. Curl verified the root returns `200`, the new JS bundle is served, and the Railway API still returns OpenAI live status with `Access-Control-Allow-Origin: https://aether-architecture-lab.sreenath-mm89.chatgpt.site`.
...
- [x] **M232.12 — The scenario a reviewer is reading, unclipped** `DONE`
  - The four scenario names shared their row with the trace control, so at 1274px they had 438px of a 654px bar and the _selected_ tab was cut mid-word. Twice before this was fought by tightening padding — treating the symptom. The tabs own their row now and the trace sits beside the future label, where it reads as a canvas-level action rather than a fifth tab.
  - The names still fit their row _exactly_ on two of the four systems, so one longer component name would clip again: the strip wraps rather than scrolls. Verified past the limit — two rows, nothing clipped, nothing truncated, no overflow.
- [x] **M232.13 — The decision strip, sized to its content** `DONE`
  - `1fr 1fr max-content` sized the three opening facts to the viewport: on a 1920px monitor each fact got a 728px cell to hold ~250px of text, so the strip read thin and stretched where at 1274px it read dense. The columns cap at 26rem, **measured rather than chosen** — longest state sentence 378px, longest eyebrow 369px. The cap only bites on large monitors; at 1280px the columns shrink and the row fills the width as before.
- [x] **M232.14 — The decision labels advance past the decision** `DONE`
  - The strip a reviewer reads first said "ready for a decision — waiting on **the reviewer**" through three distinct stages, and the hero said "Decision now: **Review** Highest resilience" after the branch had been reviewed, approved _and_ committed. Both chains tested only whether approval was _possible_, never what had already happened — and approving is the reviewer's own act.
  - One catch found only in the browser: on arrival the active branch **is** the committed baseline, so a bare `merged` test claims a decision before one exists. It briefly shipped as "Decision made → Create repair futures". The branch count guards it, and the test pins the ordering.
  - Walked end to end on two systems: arrival, branched, eligible, approved, committed now read distinctly across all three labels, with tools 13 → 8 on commit.
  - **The gate-reason test sliced a fixed 900 characters** and failed the moment the branch chain grew, with every clause it asserts still present. It slices to the end of the element now — a test that restates its scope by character count is the same failure as one that infers it.
- [x] **M232.15 — A burst of agent writes no longer conflicts with itself** `DONE`
  - Every state change fired a PUT carrying the version the page last saw, so a burst of agent writes sent several at once: the first won, the rest returned 409, and the conflict handler adopted the server's copy — which held an **earlier write from the same burst**.
  - Measured on the deployed origin: a three-write repair reached version 5 with 10 operations and settled back at version 3 with 8, losing two property changes and the approval that rested on them. A single write never showed it; only a burst does, which is exactly what an agent repairing a system produces. The local build could never show it — there is no server to conflict with.
...
- [x] **M233.9 — The bottom fade appears only where content overflows** `DONE`
  - Applied to every scrolling region, then measured: on a record that _fits_, the agent's "93.96% availability · 46m recovery" sat 14px above the edge inside a 16px fade — dimmed on a panel with nothing to scroll to. CSS cannot ask whether an element overflows, so each panel reports it and the mask keys off that; a `ResizeObserver` re-measures. Verified live: the fade appears the moment a fourth note outgrows the box, and not before.
  - The gate refuses both halves — a missing conditional rule, and any scrolling panel that masks unconditionally.
...
- [x] **M234.5 — The two judgment calls that held this at 9.5** `DONE`
  - **The branched headline set three lines where two were tighter.** Measurable: at 852px it stranded "commits." alone on a third line at 235px while the two lines above each left 200px unused. The hero's panel column had a 300px floor for content 282px wide — the row was costing the headline a line for space the panel never used. It takes what it needs now; the headline takes the rest, 939px.
  - That fixed three of four states. The committed headline stayed at three because "The record shows who decided, and on what evidence" is what the decision record below says at length, _with the evidence attached_. "The whole decision is on the record" sets two lines of 912 and 808 — measured against three alternatives, not chosen.
...
## Milestone 241 — It reads as live, and the distinction lives in the README

Every figure carried a footnote about how it was arrived at — _"implied · 5 stated capacities"_, _"These numbers are assumptions"_, _"Mean over the window, not a peak"_, _"nothing to simulate against"_. Each was true, and together they made a room looking at its own system read like a paper about a room.

- **The interface reads as what it is.** `live · 5 components reporting`, `awaiting telemetry` where nothing is reporting, `Standing by` where a thread has nothing to model yet, and `Demand: 220 rps · 2026-08-23 to 2026-08-29` with the window still attached to the number.
- **The distinction moved rather than disappeared.** `README.md` carries a section naming exactly which readings are live — repository files, OpenAI/GitHub/npm/Cloudflare status, published download volume — and what is modelled: availability, recovery and latency from a deterministic engine, reproducible by fingerprint rather than measured from production. A reviewer acting on a number does not want that on the number; a reviewer deciding whether to trust the product does.
- **Verified across arrival, branched, war room and blank canvas, then again in production:** zero occurrences of the old language, and every sentence over 55 characters that survives is a thread summary, a decision note, the seeded injection, or one state explanation.

## Milestone 242 — Evals that call the real tools, and the bugs they found

Four hundred and eleven unit tests were green while the war room wrote to a property no tool has ever accepted. A test that asserts a field name I chose passes whether or not the product agrees with me, which is why none of them caught it.

- [x] **M242.1 — An eval suite that drives the shipped registry** `DONE`
  - Fifteen checks, each one calling the real tools against real state and reading the JSON back: the surface count at each state, the absence of any approval tool, a write refused for an unknown property and the same write landing when correct, branch isolation, run-to-run determinism, the cost ceiling, output budget, concurrent rebuilds, telemetry at the component's own scale, a live source, and the proxy refusing an address it was handed. `npm run evals`. **15/15 against the deployed origin**, live network checks included.
  - Each line states the question a reader would ask and the value observed, so the output reads as evidence rather than a pass count. Two checks assert their own preconditions — a ceiling that failed to apply, or a run reporting nothing, would otherwise read as a pass.
  - Writing it found three more of my own errors within the hour: `deriveGraph` takes a branch and not an id, a scenario reports `availability` and `inputHash` rather than the names I had written down, and a cost ceiling is set with `amountUsd`. Every one would have passed as a unit test against a mock.

**Five bugs, all found by measuring rather than by testing.**

- **The reading advised a property the tool refuses.** `read_component_telemetry` ended every reply with _"propose_architecture_change with peakRps"_. There is no `peakRps` on that tool. The war room did what the reading told it to and was refused every time, silently, while reporting the reading as applied. Peak is what telemetry observed and an agent does not get to rewrite an observation; capacity is what a reading moves, and only upward.
- **Telemetry read the wrong scale.** A ledger declaring 12,000 RPS was read as a generic 4,000 RPS database, so the suggested capacity came out _below_ what was already provisioned and the agent's own fix made the deficit larger — 4,153 RPS became 10,153 — while the board reported it as progress. Readings are anchored to what the component states about itself.
- **The reading read the wrong branch.** Provisioned capacity came from whichever branch was active while the write targeted a repair that had already raised it, so the guard against shrinking a component compared the wrong numbers and let a session store go from 288,000 to 196,386. The tool takes a `branchId` now.
- **One future, two deficits, no way to tell them apart.** 113,771 RPS beside 8,771 RPS, both correct — the first under a traffic spike at 1.5x demand, the second under a regional outage — and the board named neither. Every finding now carries the future, the version, and the scenario. The propose step was also pinned to a hardcoded branch while the validate step used the active one, so a room comparing three futures could report one future's evidence as another's.
- **`InvalidStateError: Duplicate tool name` in production.** Joining the room while a branch was being created threw, and the exception aborted the branch creation with it: ten tools and no futures, on a product whose central claim is that the surface follows the architecture. Two rebuilds arriving with different states both tore the surface down and the second re-registered a name the first had put back. Refreshes queue now.
  - **The regression check took three attempts to become honest.** It passed against the fix and against its absence twice over — first because the stub registrar accepted duplicate names where a browser refuses them, then because it registered without yielding, so two rebuilds interleaved perfectly and never collided. It only reproduces with refreshes that disagree about the state, which is what a room actually produces.

## Milestone 243 — A system that fails the way a live event fails

The three shipped systems all fail the way steady-state systems fail: something breaks and the load stays where it was. A live event fails the other way — the load arrives faster than capacity can, and the bottleneck is lead time rather than headroom.

- [x] **M243.1 — Live event streaming** `DONE`
  - Shaped from what Hotstar published about the 2019 World Cup semi-final at AWS re:Invent (CMY302): 25.3M peak concurrent, over 1M requests per second, 10 Tbps out. The useful part of that talk is why they do not autoscale into a spike — an instance takes about a minute to boot and the scaler reacts in about ninety seconds, while the audience grows by more than a million people a minute.
  - So the fixture's spike originates at the licence service: every viewer asks for a licence before the first frame plays, which makes a resumed innings a single burst against the thinnest margin on the board. **Verified live:** `traffic_spike` reports it short by 75,000 RPS and traces the cascade through the edge tier to the ad queue and telemetry. All four incident threads derive correctly on it, which is the evidence that the derivation is general rather than fitted to one fixture.
  - Region bands are bounding boxes around their members, so two regions sharing horizontal space drew their labels over each other — the canvas read `AP-SOUTH-1SINGAPORE` until the regions were split by row.
- [x] **M243.2 — `docs/DATA_SOURCES.md`** `DONE`
  - What is read live, what this application generates, and what is modelled, plus the published material the fixtures are shaped from. It also records two figures that circulate widely and are wrong, so nobody reaches for them later: an "821 million concurrent" claim that exceeds India's internet population against the ICC's published 72.5M, and a "500,000 orders per minute" figure absent from the AWS post it is credited to.

## Milestone 244 — Premortem: what would have lost it

Assume the entry lost, then go find why. Two agents audited the first-run and error paths and the documentation against the code while the deployed product was measured directly.

- [x] **M244.1 — The workspace outgrew what the server accepts** `DONE`
  - The finding that would have lost it. A shared room on the deployed origin had reached **2,340 audit entries and 1.04 MB against a 1 MB ceiling**, so every further action by anyone holding that link was refused — and reported as `Offline draft`, the same words as a network blip, on a failure that can never clear. A product whose claim is a durable auditable record had an audit log that made the record unsavable.
  - Two structures grew without bound. The audit array is now held to 1,500 entries with the retired count kept in the workspace, and entries beyond the most recent 200 drop their `input`/`result` payloads. Branch operations collapse superseded writes in place — `deriveGraph` replays in order, so only the last write to a target matters, but a branch had reached **5,006 operations and 416 KB** because dragging a card wrote one per step.
  - **Measured in the browser, not in a test:** 414,609 bytes after 400 tool writes, 414,610 after 1,600. Flat.
  - **The live room recovered rather than only stopping the bleeding:** loaded at 1.15 MB, bounded on the way in, wrote, and reports `Synced`. Server copy went **999,774 → 518,733 bytes**.
  - Three faults surfaced while proving it, each caught by measuring the running product after the previous fix looked right: `mergeEvidence` unions audits so a bound applied only on dispatch was undone by the next reconcile (climbed back to 5,764); the merge key hashed `input`, which compaction removes, so a compacted entry and its full twin keyed differently and the audit doubled every reconcile; entry and revision ids counted `audit.length`, which stops advancing once bounded, so 1,500 entries were all `event-1501`.
  - A 413 is reported as itself now — `Too large to save`, with what to do about it.
- [x] **M244.2 — A wrong `?system=` link handed over somebody else's workspace** `DONE`
  - `requestedTemplate()` returned `undefined` both for "no link" and for "a link naming something we do not ship", and those want opposite behaviour. A judge following a slightly wrong link — a typo, a stale URL out of a document — landed on a live-event workspace with four branches and eighteen tools already open, which is the middle of a story rather than the start of one. Verified: `?system=nope` now opens the default system fresh at ten tools.
- [x] **M244.3 — The demo script pointed the camera at text that does not exist** `DONE`
  - `docs/DEMO.md` and `docs/SUBMISSION.md` — the shot list for the required video — told a recorder to point at `WebMCP live · 5 state-aware tools` and `13 state-aware tools`. The interface has not rendered "state-aware tools" since it was removed as developer vocabulary on a product surface; it reads `N things it may do here`. The counts were wrong too: ten and eighteen, not five and thirteen. A judge reading the submission beside the live app would have seen different text and different numbers in the opening frame.
  - `docs/WEBMCP_COMPLIANCE.md` told a judge running its own verification procedure to expect `5` and `12` from `getTools()`. The registry publishes **10** and **18** — and the same document already said so four rows above.
  - Also corrected: "all fourteen" schemas (eighteen), "seven write tools" (eight), a comparison payload documented as 1,528 characters that **measures 1,909**, `docs/ARCHITECTURE.md` still saying two starting systems when four ship, two capability-table rows claiming `run_failure_scenario` and `add_decision_note` need a repair future when both register on an editable blank canvas, and a Lighthouse claim naming "agentic browsing", which is not a Lighthouse category.
- [x] **M244.4 — The drift guard stopped one word short** `DONE`
  - The test that exists to catch exactly these errors spelled numbers `five` through `thirteen`, so every wrong count above thirteen was invisible to it — which is why four of the above survived. It now runs through `eighteen` and scans `ARCHITECTURE.md` and `DEMO.md`, neither of which it had ever read. Two demo-script tests were themselves pinning the wrong strings in place, asserting the script still said `10 state-aware tools`; they assert the measured chip text now.
