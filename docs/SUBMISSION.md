...
- State-dependent registration: five read and branch tools on a committed architecture, ten once the model is editable — a repair future is open or the reviewer is building their own system — and thirteen when both hold. Component and region enums are enumerated from the live graph rather than a fixed list.
