...
7. **After the commit.** Eleven tools registered, `Rollback this merge`
...
- State-dependent registration: eight read and branch tools on a committed architecture, thirteen once the model is editable — a repair future is open or the reviewer is building their own system — and sixteen when both hold. Component and region enums are enumerated from the live graph rather than a fixed list.
