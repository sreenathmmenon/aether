...
- State-dependent registration: nine read and branch tools on a committed architecture, fourteen once the model is editable — a repair future is open or the reviewer is building their own system — and seventeen when both hold. Component and region enums are enumerated from the live graph rather than a fixed list.
