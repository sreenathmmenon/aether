...
After a repair future exists, the in-app browser observed the expanded branch-gated surface: at the time of that recording it held nine tools. The surface has since grown to twelve, adding `add_architecture_component`, `model_architecture`, and `connect_components` so an agent can turn a user brief into a graph as well as tune it; `src/platform/webmcp/registry.test.ts` asserts the five-tool, ten-tool, and twelve-tool surfaces. Local end-to-end validation called `add_decision_note`, confirmed that the agent note appeared in the shared decision room, and confirmed that no approval or merge capability is registered.
